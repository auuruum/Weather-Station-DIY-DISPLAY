...
bool fetchForecastData() {
    ...
    // Open-Meteo API URL for Vilnius, Lithuania (54.6872°N, 25.2797°E)
    // You can change latitude/longitude to your location
    // Request current weather to get current time, and forecast_hours=24 to get enough future hours
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        // Allocate larger JSON document for forecast data
        JsonDocument doc;
        DeserializationError error = deserializeJson(doc, payload);
        
        if (!error) {
            // Get current time from API to filter past hours
            String currentTime = doc["current"]["time"] | "";
            
            // Parse hourly forecast (next 12 hours from now)
            ...
            // Find the starting index (skip past hours)
            int startIndex = 0;
            if (currentTime.length() > 0) {
                // Current time format: "2026-01-08T20:00"
            ...
            // Store next 12 hours from current time
